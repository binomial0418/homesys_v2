...
class g3sensor():
    ...
    def check_keyword(self):
        ...
        while True:
            ...
            #轉成16進制 的python3寫法    
            token_hex=str(binascii.hexlify(token)).strip('b')
            if token_hex == "'42'":
                if debug: print("get 42")
                token2 = self.serial.read()
                token2_hex=str(binascii.hexlify(token2)).strip('b')
                ...
                if token2_hex == "'4d'":
                    ...
                    token3_hex=str(binascii.hexlify(token3)).strip('b')
                    if token3_hex == "'4d'":
                       if debug: print("get 4d")
                       return True
    ...
    def read_data(self):
        ...
        data_hex=str(binascii.hexlify(data)).strip('b')
...
if __name__ == '__main__': 
    ...
    while True:
       ...
       if pmdata != 0:
          print(pmdata)
          print('PM1.0:[' , pmdata[3],']  PM2.5:[',pmdata[4],
                ']  PM10:[',pmdata[5],']')
          break
