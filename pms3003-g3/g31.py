...
class g3sensor():
    ...
    def check_keyword(self):
        ...
        while True:
            ...
            token_hex = int.from_bytes(token, byteorder='big')
            print('token_hex=',token_hex)
            if token_hex == '42':
                if debug: print("get 42")
                token2 = self.serial.read()
                token2_hex = int.from_bytes(token2, byteorder='big')
            ...
            print('token3=',token)
            token3_hex = int.from_bytes(token3, byteorder='big')
            print('token3_hex=',token3_hex)
            #if token3_hex == '4d':
            if token3_hex == 148:
               if debug: print("get 4d")
               return True
